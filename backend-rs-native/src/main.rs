...
use config::{
    ...
    range_expr_max_tokens, range_text_max_len, request_body_limit_bytes, resolve_static_root,
    sim_iteration_cap_max, sim_max_runtime_ms_env, sim_runtime_cap_ms, sim_runtime_default_ms,
    try_acquire_heavy_request_permit,
};
...
use static_assets::static_cache_headers;
...
impl PoolHand {
    fn from_slice(hand: &[u8]) -> Self {
        debug_assert!(hand.len() <= 6);
        let mut cards = [0u8; 6];
    ...
    fn as_slice(&self) -> &[u8] {
        &self.cards[..usize::from(self.len)]
    }
...
fn collect_exact_pool_with_limit_parallel(
    ...
    let mut pool = (0..=max_start)
        ...
        .map(|first_idx| {
            ...
            let mut local = Vec::<PoolHand>::new();
        ...
        .reduce(Vec::<PoolHand>::new, |mut acc, mut part| {
            acc.append(&mut part);
            acc
        });
...
fn collect_exact_pool_with_limit_rec(
    ...
    if depth == hand.len() {
        if range_expr_match(expr, hand, board) {
            ...
            pool.push(PoolHand::from_slice(hand));
        }
        return;
    }
...
fn collect_exact_pool_with_limit_parallel_rec(
    ...
    if depth == hand.len() {
        if range_expr_match(expr, hand, board) {
            ...
            local.push(PoolHand::from_slice(hand));
        }
        return;
    }
    ...
    enumerate_hands(base, hand_size, |hand| {
        if range_expr_match(expr, hand, board) {
            ...
            if pool.len() < cap {
                ...
                if j < cap {
                    pool[j] = PoolHand::from_slice(hand);
                }
            }
        }
    });
        ...
        players: payload
            ...
            .map(|p| native_sim::SimPlayerReq {
                ...
                pool: p
                    .pool
                    .as_ref()
                    .map(|pool| pool.iter().copied().map(PoolHand::to_vec).collect()),
                plan: p.plan.clone(),
                range_text: None,
